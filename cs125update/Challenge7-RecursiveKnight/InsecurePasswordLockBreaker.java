//UIUC CS125 FALL 2013 MP. File: InsecurePasswordLockBreaker.java, CS125 Project: Challenge7-RecursiveKnight, Version: 2013-11-12T10:44:04-0600.496641397
public class InsecurePasswordLockBreaker {
	...
	public static char[] breakLock(InsecurePasswordLock lock) {
		...
		while(lock.open(key) == -1)
			key = new char[key.length + 1];
		for(int i = 0; i < key.length; i ++)
			key[i] = 'A';
		while(lock.open(key) < key.length){
			 key[lock.open(key)] ++;
		 }
		return key;
	}
