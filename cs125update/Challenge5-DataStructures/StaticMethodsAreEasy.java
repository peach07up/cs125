//UIUC CS125 FALL 2013 MP. File: StaticMethodsAreEasy.java, CS125 Project: Challenge5-DataStructures, Version: 2013-10-14T15:42:09-0500.735397010
/* 
 * @author pliu15
 */
...
// Hint: Get the Geocache class working and passing its tests first.
	...
	public static Geocache[] createGeocaches(int num) {
		if (num < 0) num = 0;
		Geocache[] newGeocache = new Geocache[num];
		for (int i = 0; i < num; i ++) {
			double x = Math.random()*100;
			double y = Math.random()*100;
			newGeocache[i] = new Geocache(x, y);
		}
		return newGeocache;	
	}
	public int length(){
...
//write the method here...
	...
	public static int ensureMinimumXValue(Geocache[] p, double minX) {
		...
		for (int i = 0; i < p.length; i ++){
			if (p[i].getX() < minX) p[i].setX(minX);
			count ++;
		}
		return count;
	}
